        ...
        Some(token) => {
            let claims = match auth_service.verify_token(token) {
                ...
                Ok(None) => {
                    return (
                        ...
                        Json(serde_json::json!({"code": 401, "message": "Invalid token"})),
                    )
                        .into_response();
                }
                Err(e) => {
            ...
            let user_id = match uuid::Uuid::parse_str(&claims.user_id) {
                ...
                Err(_) => {
                    return (
                        StatusCode::UNAUTHORIZED,
                        Json(serde_json::json!({"code": 401, "message": "Invalid token claims"})),
                    )
                        .into_response();
                }
            };
            ...
            request
                .extensions_mut()
                .insert(UserContext { user_id, tenant_id });
            next.run(request).await
        }
        None => (
            StatusCode::UNAUTHORIZED,
            Json(serde_json::json!({"code": 401, "message": "Missing authorization token"})),
        )
            .into_response(),
    }
}
...
impl AppState {
    pub async fn new(config: crate::config::AppConfig) -> anyhow::Result<Self> {
        ...
        let auth_service =
            AuthService::new(registry.db_dao.clone(), Arc::new(config.server.clone()));
